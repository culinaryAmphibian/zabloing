...
let errEmbed = {color: global.orange, title: 'error', description: 'you don\'t have the permissions!', footer: global.footer};
...
module.exports =
{
    ...
    execute(message, args, bot)
    {
        ...
        if (!cmdToToggle) return message.channel.send({embeds: [errEmbed]});
        errEmbed.description = 'you can\'t disable that command!';
        if (['enable', 'disable', 'turnoff', 'turnon', 'toggle', 'switch'].includes(cmdToToggle)) return message.channel.send({embeds: [errEmbed]});
        ...
        switch(args[0])
        {
            case 'enable', 'turnon':
                ...
                if (!ServerJSON[message.guild.id].disabled) return message.channel.send({embeds: [errEmbed]});
                errEmbed.description = 'that command isn\'t disabled.'
                if (!ServerJSON[message.guild.id].disabled.includes(cmdToToggle)) return message.channel.send({embeds: [errEmbed]});
