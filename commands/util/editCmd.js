...
let errEmbed = {color: global.orange, title: 'error', description: 'this guild doesn\'t have commands in the first place!', footer: global.footer};
...
module.exports =
{
    ...
    execute(message)
    {
        if (!ServerJSON[message.guild.id].cmds) return message.channel.send({embeds: [errEmbed]});
        ...
        collector.on('end', collected =>
        {
            ...
            if (!cmdToEdit) return message.channel.send({embeds: [errEmbed]});
            errEmbed.description = 'please use the editalias command to edit aliases.';
            if (cmdToEdit.response.startsWith('[pref]')) return message.channel.send({embeds: [errEmbed]});
            let changeToValue = collected[questions.length - 1]?.content;
            errEmbed.description = 'no bad words!';
            if (SecretJSON.find(bw => changeToValue.split(" ").includes(bw))) return message.channel.send({embeds: [errEmbed]});
            switch(collected[1].content.toLowerCase())
            {
                case 'trigger', 't', 'trig':
                    errEmbed.description = 'the trigger has to be a string.';
                    if (!changeToValue) return message.channel.send({embeds: [errEmbed]});
                ...
                case 'response', 'r', 'res':
                    ...
                    if (changeToValue.startsWith('[pref]')) return message.channel.send({embeds: [errEmbed]});
                ...
                case 'd', 'del', 'delete':
                    ...
                    if ((message.author.id !== cmdToEdit.author) || (message.author.id !== message.guild.ownerID)) return message.channel.send({embeds: [errEmbed]});
