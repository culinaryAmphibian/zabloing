...
module.exports =
{
    ...
    async execute(message, args)
    {
        ...
        else if (hexMatch) col = hexMatch[1];
        else if (args[1].toLowerCase() == 'random') col = 'RANDOM';
        ...
        if (membersearchFrom) guildMember.roles.remove(membersearchFrom, reason);

        let guildSearchFrom = message.guild.roles.cache.filter(r => hexMatching(r.name) == r.color.toString(16))
                                .find(r => r.name.slice(1) == col);
        ...
        if (col == 'RANDOM') roleToGive.edit({name: `#${roleToGive.color.toString(16)}`});
